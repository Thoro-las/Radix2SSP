...
char *tobinaryl(uint64_t number, int length, int* ones) {
  if (ones) *ones = 0;
  if (length > 64)
    length = 64;
...
char *tobinary(uint64_t number, int *ones) { return tobinaryl(number, 64, ones); }

// Extracts 'length' bits from the binary representation of 'number' starting at
// 'start' from LSB
uint32_t extract(uint64_t number, int start, int length) {
  return (number >> start) & ((1 << length) - 1);
}

// Returns the bit length of a number
int bitlength(uint64_t number) {
  int length = 0;
